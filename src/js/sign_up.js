...
function sign_up(){
   ...
   var master_and_derived_key = CryptoJS.PBKDF2(`${email}:${password}`,salt,{iterations:50000,keySize:32})
   var master_and_derived_key_string = master_and_derived_key.toString()
   console.log(master_and_derived_key_string)
   var master_enc_key_string = master_and_derived_key_string.slice(0,master_and_derived_key_string.length/2)
   var master_enc_key = CryptoJS.enc.Hex.parse(master_enc_key_string)
   var derived_key_string = master_and_derived_key_string.slice(master_and_derived_key_string.length/2,master_and_derived_key_string)
   var derived_key = CryptoJS.enc.Hex.parse(derived_key_string)
   var master_key = CryptoJS.lib.WordArray.random(16)
